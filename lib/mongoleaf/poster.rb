    ...
    module REST
      ...
      def post hash = nil
        ...
        net.start {|http| http.request(request)}.tap do |r|
          ...
          Hash.new.store :response, SocketError
      end
      ...
      def get_response response
        case response
          when Net::HTTPBadResponse
            Hash.new.store :response, Net::HTTPBadResponse
          when Net::HTTPRedirection
            Hash.new.store :response, Net::HTTPRedirection
          else
            JSON.parse(response.tap { |r| 
              puts "#{r.class}:#{r.code}:#{r.message}"
